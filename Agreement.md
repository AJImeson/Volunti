...
- In GitLab we don't have pull requests, we have merge requests, works exactly the same.

- If you're used to working in a terminal, the commands will be exactly the same. If working with VS-code, use GitLab workflow extension to add/commit etc.

- As decided during kick-off: We see to that we work on our own branches and not directly on main-master. If we edit another persons work/code we notify about this. 

* Collab software:

- We will be working Agile in Jira (Decided at kick-off). Here we can plan our sprints and set tasks that needs to be handled.
